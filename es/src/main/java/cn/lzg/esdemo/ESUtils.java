...
public class ESUtils {
	 ...
	 */
	public static void createIndex(List<Goods> goodsList)
			throws UnknownHostException, JsonProcessingException {
		Client client = getClient();
		// 如果存在就先删除索引
		if (client.admin().indices().prepareExists("test_index").get()
				.isExists()) {
		...
		client.admin().indices().prepareCreate("test_index")
				.addMapping("goods", mappingStr).get();
		...
		for (Goods goods : goodsList) {
			...
			bulkRequest.add(new IndexRequest("test_index", "goods", goods
					.getId() + "").source(json));
		}
		...
		if (bulkResponse.hasFailures()) {
			System.out
					.println("====================批量创建索引过程中出现错误 下面是错误信息==========================");
			long count = 0L;
			for (BulkItemResponse bulkItemResponse : bulkResponse) {
				System.out.println("发生错误的 索引id为 : " + bulkItemResponse.getId()
						+ " ，错误信息为：" + bulkItemResponse.getFailureMessage());
				count++;
			}
			System.out.println("====================批量创建索引过程中出现错误 上面是错误信息 共有: "
					+ count + " 条记录==========================");
		}
	...
	public static void addDocument(String index, String type, Goods goods)
		...
		client.prepareIndex(index, type, goods.getId() + "").setSource(json)
				.get();
	 ...
	 * @throws UnknownHostException
	 */
	public static void deleteDocument(String index, String type, Long goodsId)
			throws UnknownHostException {
		Client client = getClient();
