...
public class PrecisePromotionParseResultRow {
	...
	public static PrecisePromotionParseResultRow buildByTableColumn(TableRow row) {
		...
		resultToRet.promotionTime = StringEscapeUtils.unescapeHtml(StringUtil.trim(promotionTime));

		resultToRet.adInfoId = StringEscapeUtils.unescapeHtml(StringUtil.trim(adInfoId));
		resultToRet.costInfoId = costInfoId;
		resultToRet.promotionBudget = StringEscapeUtils.unescapeHtml(StringUtil.trim(promotionBudget));
		resultToRet.clickPrice = StringEscapeUtils.unescapeHtml(StringUtil.trim(clickPrice));
		resultToRet.showClickInfo = StringEscapeUtils.unescapeHtml(StringUtil.trim(showClickInfo));
		resultToRet.leftBudget = StringEscapeUtils.unescapeHtml(StringUtil.trim(leftBudget));
		resultToRet.payWay = StringEscapeUtils.unescapeHtml(StringUtil.trim(payWay));
		resultToRet.operation = StringEscapeUtils.unescapeHtml(StringUtil.trim(operation));
